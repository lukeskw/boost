...
test('removes preceding indentation from guideline bullet points', function () {
    // Test the static composeGuidelines method directly with various indentation scenarios
    $guidelines = collect([
        'indentation-test' => "    - Four spaces before dash
        - Eight spaces before dash
  - Two spaces before dash
\t- Tab before dash
    \t- Spaces and tab before dash
- No indentation
Regular text with    spaces preserved
        - Another indented item
{{TEST_MARKER_SHOULD_BE_PRESERVED}}
    - {{INDENTED_TEST_MARKER}}",
    ]);

    $result = GuidelineComposer::composeGuidelines($guidelines);

    // Verify all bullet points have no leading whitespace
    $lines = explode("\n", $result);
    $bulletLines = array_filter($lines, fn ($line) => str_starts_with(trim($line), '-'));

    foreach ($bulletLines as $line) {
        // All bullet points must start at the beginning of the line
        expect($line)->toMatch('/^-/');
    }

    // Check specific transformations
    expect($result)
        ->toContain('- Four spaces before dash')
        ->toContain('- Eight spaces before dash')
        ->toContain('- Two spaces before dash')
        ->toContain('- Tab before dash')
        ->toContain('- Spaces and tab before dash')
        ->toContain('- No indentation')
        ->toContain('- Another indented item')
        ->toContain('- {{INDENTED_TEST_MARKER}}')
        // Non-bullet lines should preserve internal spacing
        ->toContain('Regular text with    spaces preserved')
        // Test markers should be preserved
        ->toContain('{{TEST_MARKER_SHOULD_BE_PRESERVED}}')
        // No bullet point lines should start with spaces followed by dash
        ->not->toContain('    -')
        ->not->toContain('  -')
        ->not->toContain("\t-");
});
